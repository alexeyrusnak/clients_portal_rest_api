CREATE OR REPLACE PACKAGE SBC_MESSAGE AS
  ...
  ******************************************************************************/
  ...
  -- Удаляет лишние тэги
  FUNCTION READ_BODY_MSG_F(P_MESSAGE_TEXT MESSAGES2CUSTOMERS.MESSAGE_TEXT%TYPE)
    RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY SBC_MESSAGE AS
  ...
  PROCEDURE SET_PARAM_EMAIL IS
    ...
    begin
      ...
      if Value_Param = '-' then
        HandleError(-20000,
                    'Не найден настроечный параметр:"' ||
                    UPPER(Trim(Name_Param)) || '"',
                    'SET_PARAM_EMAIL');
  ...
  */
  ...
  begin
    BEGIN
      ...
      SET_PARAM_EMAIL;
      if (trim(P_MES_EMAIL_FROM) IS NOT NULL) then
        MES_EMAIL_FROM := P_MES_EMAIL_FROM;
      end if;
      if (trim(P_MES_EMAIL_SERVER) IS NOT NULL) then
        MES_EMAIL_SERVER := P_MES_EMAIL_SERVER;
      end if;
      if (trim(P_MES_EMAIL_PASSW) IS NOT NULL) then
        MES_EMAIL_PASSW := P_MES_EMAIL_PASSW;
      end if;
      if (trim(P_MES_EMAIL_PORT) IS NOT NULL) then
        MES_EMAIL_PORT := P_MES_EMAIL_PORT;
      end if;
      if (trim(P_MES_EMAIL_NAME) IS NOT NULL) then
        MES_EMAIL_NAME := P_MES_EMAIL_NAME;
      end if;
      if (P_MES_EMAIL_AUTH IS NOT NULL) then
        MES_EMAIL_AUTH := P_MES_EMAIL_AUTH;
      end if;
      --*******************************************************************************************************************
  ...
  BEGIN
    ...
    -- Стандартные исключения Oracle и доморощенные в SBConsulting
      WHEN -6530 then
        RaiseError('Попытка доступа к неинициализированной переменной (ACCESS_INTO_NULL)',
                   pErrCode);
      WHEN -6592 then
        RaiseError('Не найдено значение в операторе CASE (CASE_NOT_FOUND)',
                   pErrCode);
      WHEN -6511 then
        RaiseError('Попытка открыть открытый курсор (CURSOR_ALREADY_OPEN)',
                   pErrCode);
      WHEN -1 then
        RaiseError('Запись уже существует (DUP_VAL_ON_INDEX)',
                   pErrCode);
      WHEN -1001 then
        RaiseError('Ошибочная операция с курсором (INVALID_CURSOR)',
                   pErrCode);
      WHEN -1722 then
        RaiseError('Ошибка преобразования строки в число (INVALID_NUMBER)',
                   pErrCode);
      WHEN 100 then
        RaiseError('Не найдены записи, удовлетворяющие условию (NO_DATA_FOUND)',
                   pErrCode);
      WHEN -6501 then
        RaiseError('Внутренняя ошибка PL/SQL (PROGRAM_ERROR)',
                   pErrCode);
      WHEN -6504 then
        RaiseError('Не совпадение типов (ROWTYPE_MISMATCH)',
                   pErrCode);
      WHEN -6500 then
        RaiseError('Ошибка памяти (STORAGE_ERROR)', pErrCode);
      WHEN -1410 then
        RaiseError('Неправильный системный номер записи (SYS_INVALID_ROWID)',
                   pErrCode);
      WHEN -51 then
        RaiseError('Время ожидания ресурса истекло (TIMEOUT_ON_RESOURCE)',
                   pErrCode);
      WHEN -1422 then
        RaiseError('Слишком большое количество записей (TOO_MANY_ROWS)',
                   pErrCode);
      WHEN -6502 then
        RaiseError('Ошибка присвоения значения переменной (VALUE_ERROR)',
                   pErrCode);
      WHEN -1476 then
        RaiseError('Деление на ноль (ZERO_DIVIDE)', pErrCode);
      WHEN -2292 then
        RaiseError('Существуют порожденные записи (INTEGRITY_CONSTRAINT)',
                   pErrCode);
      ELSE
  ...
  Begin
    ...
    For DAN in (Select DEF
                  from table(cast(SBC_MESSAGE.GET_VALUES_MESSAGES as
                                  tbl_VALUES_MESSAGES))) loop
    ...
    if INSTR(V_MESSAGES_TEXT, '<') <> 0 Then
      RaiseError('Неверный текст шаблона, переменная (' ||
                 SUBSTR(V_MESSAGES_TEXT,
                        INSTR(V_MESSAGES_TEXT, '<'),
                        INSTR(V_MESSAGES_TEXT, '>') -
                        INSTR(V_MESSAGES_TEXT, '<') + 1) ||
                 ')не найденна в списке переменных.');
    end if;
  ...
  begin
    ...
    i := 0;
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<номер заказа>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата заказа>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата выпуска ГТД>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата отгрузки>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата выгрузки>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    ...
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<грузоотправитель>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<город отправки>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата погрузки>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    ...
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<порт прибытия>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<город прибытия>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<ожидаемая дата>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата прибытия>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата убытия>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    ...
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<внутренний номер заказа>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<планируемая дата прибытия>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    ...
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<номер авто к клиенту>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<фамилия к клиенту>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата убытия к клиенту>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<телефон к клиенту>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<дата подачи>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
    P_VALUES_MESSAGES(i) := t_VALUES_MESSAGES('<груз по заявке>',
                                              null);
    i := i + 1;
    P_VALUES_MESSAGES.extend;
  ...
  BEGIN
    ...
    IF (INSTR(V_MSG_INFO, '<дата заказа>') > 0) OR
       (INSTR(V_MSG_INFO, '<номер заказа>') > 0) THEN
      BEGIN
    ...
    IF (INSTR(V_MSG_INFO, '<дата выпуска ГТД>') > 0) THEN
      BEGIN
        select date_out
          INTO V_DATE_OUT_GTD
          from GTDS
         where GTD_ID in (select GTD_GTD_ID
                            from gtd_payments
                           where ORD_ORD_ID = P_ORD_ID);
      EXCEPTION
        WHEN others THEN
          V_DATE_OUT_GTD := null;
      END;
      ...
      BEGIN
        SELECT OW.DATE_IN
         ...
         WHERE OW.ORD_ORD_ID = P_ORD_ID
           AND OW.ORWS_TYPE = 1
           AND OW.DATE_IN IS NOT NULL
           AND OW.DEL_DATE is null;
      EXCEPTION
        WHEN OTHERS THEN
          V_DATE_IN := null;
      END;
    ...
    IF (INSTR(V_MSG_INFO, '<дата выгрузки>') > 0) THEN
      BEGIN
        select voch_date
          into V_VOCH_DATE
          from VOUCHERS
         where VOCH_ID in (select VOCH_VOCH_ID
                             from voch_details
                            where ord_ord_id = P_ORD_ID);
      EXCEPTION
        WHEN OTHERS THEN
    ...
    IF (INSTR(V_MSG_INFO, '<склад>') > 0) THEN
      BEGIN
        ...
        FOR DAN in (select s.def
                     ...
                     where OW3.ORD_ORD_ID = P_ORD_ID
                       AND OW3.ORWS_TYPE = 3
                       AND OW3.DEL_DATE is null
                       and OW3.orws_id = cm.orws_orws_id
                       and cm.port_port_in_id = s.port_id(+)
    ...
    IF (INSTR(V_MSG_INFO, '<номер авто к клиенту>') > 0) OR
      ...
      BEGIN
        select cr.state_number,
               c.DRIVER_NAME,
               c.DRIVER_PHONE,
               NVL(c.PORT_OUT, C.DATE_OUT)
         ...
         where c.cmr_id = (select distinct first_value(od.cmr_id) over(order by od.cmr_id desc)
                            ...
                            where ow3.ord_ord_id = P_ORD_ID
                              AND ow3.orws_type = 3
                              AND ow3.del_date is null
                              AND ow3.orws_id = od.orws_orws_id)
           and c.car_car_id = cr.car_id(+)
           and c.del_date is Null;
      EXCEPTION
        WHEN OTHERS THEN
          V_CAR_NUMBER_C   := NULL;
          V_DRIVER_NAME_C  := NULL;
          V_DRIVER_PHONE_C := NULL;
          V_DATE_C         := NULL;
      END;
    END IF;
             ...
             where o.ORD_ID = P_ORD_ID
               ...
               AND
                  --and k.IS_CUSTOM=1  -- Есть отметка таможни
                   K.TRSP_TRSP_ID = S.TRSP_ID(+)
               AND k.tmtb_tmtb_id = tt.tmtb_id(+)
               AND tt.trsp_trsp_id = s_f.trsp_id(+)
               AND OW.ORWS_TYPE = 1
               AND O.ORD_ID = OW.ORD_ORD_ID
               AND k.KNSM_TYPE = 1
               and rownum = 1;
      ...
      BEGIN
        select def pod_port_id_
         ...
         where port_id in (SELECT pod_port_id
                            ...
                            WHERE KD.ORD_ORD_ID = P_ORD_ID
                              AND KD.KNOR_KNOR_ID = KO.KNOR_ID(+)
                              AND KO.KNSM_KNSM_ID = K.KNSM_ID(+)
                              and is_custom = 1);
    ...
    IF (INSTR(V_MSG_INFO, '<ожидаемая дата>') > 0) THEN
      BEGIN
        SELECT NVL(KO.ETA_DATE, K.ETA_DATE)
         ...
         WHERE OW.ORD_ORD_ID = KD.ORD_ORD_ID(+)
           AND KD.KNOR_KNOR_ID = KO.KNOR_ID(+)
           AND KO.KNSM_KNSM_ID = K.KNSM_ID(+)
           AND P_ORD_ID = OW.ORD_ORD_ID
           AND OW.ORWS_TYPE = 1;
      EXCEPTION
        WHEN OTHERS THEN
          V_ETA_DATE := NULL;
      ...
      BEGIN
        select TO_CHAR(CM.PORT_OUT, 'DD.MM.YYYY HH24:MI:SS'), CM.PORT_OUT -- cmrs.PORT_OUT
         ...
         where ow3.ord_ord_id = P_ORD_ID
           ...
           and ow3.del_date is Null
           and ow3.orws_id = cm.orws_orws_id
           and cm.port_out is not Null
           and cm.del_date is Null
           and cm.cmr_id = (select min(od.cmr_id)
                              from cmrs od
                             where od.orws_orws_id = ow3.orws_id
                               and od.del_date is Null);
      EXCEPTION
        WHEN OTHERS THEN
          V_PORT_OUT    := NULL;
          V_PORT_OUT_df := NULL;
      ...
      BEGIN
        select cr.state_number, cm.driver_name, cm.driver_phone
         ...
         where ow3.ord_ord_id = P_ORD_ID
           ...
           and ow3.orws_id = cm.orws_orws_id
           and cm.del_date is Null
           and cm.cmr_id = (select max(od.cmr_id)
                              from cmrs od
                             where od.orws_orws_id = ow3.orws_id
                               and od.del_date is Null)
           and cm.car_car_id = cr.car_id(+)
           and cm.port_out is not null;
      EXCEPTION
        WHEN OTHERS THEN
          V_CAR_NUMBER   := NULL;
    ...
    IF (INSTR(V_MSG_INFO,
              '<планируемая дата прибытия>') > 0) THEN
      BEGIN
        Select Decode(Decode(NVL(TO_CHAR(DT1), '0'),
                             '0',
                             Decode(NVL(TO_CHAR(DT2), '0'),
                                    '0',
                                    NVL(TO_CHAR(DT3), '0'),
                                    NVL(TO_CHAR(DT2), '0')),
                             NVL(TO_CHAR(DT1), '0')),
    ...
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<номер заказа>',
                          V_ORD_NUMBER);
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата заказа>',
                          NVL(TO_CHAR(V_ORD_DATE, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата выпуска ГТД>',
                          NVL(TO_CHAR(V_DATE_OUT_GTD, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата отгрузки>',
                          NVL(TO_CHAR(V_DATE_IN, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата выгрузки>',
                          NVL(TO_CHAR(V_VOCH_DATE, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<контейнер>',
                          NVL(V_CONT_NUMBER, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<грузоотправитель>',
                          NVL(V_SHORT_NAME, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<город отправки>',
                          V_CITY_DEF);
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата погрузки>',
                          NVL(TO_CHAR(V_DATE_OUT, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<судно>',
                          NVL(V_SHIP_DEF, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
    ...
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<ожидаемая дата>',
                          NVL(TO_CHAR(V_ETA_DATE, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата прибытия>',
                          NVL(TO_CHAR(V_ARRIVAL_DATE, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата убытия>',
                          NVL(TO_CHAR(V_PORT_OUT_df, 'dd.mm.yyyy'), 'N/A')); -- used to be V_PORT_OUT
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<номер авто>',
                          NVL(V_CAR_NUMBER, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<фамилия>',
                          NVL(V_DRIVER_NAME, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<телефон>',
                          NVL(V_DRIVER_PHONE, 'N/A'));
    --Замена текста
    V_MSG_INFO := REPLACE(V_MSG_INFO,
    ...
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          ...
                          NVL(TO_CHAR(V_DATE_PLAN, 'dd.mm.yyyy'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO, '<склад>', NVL(V_SKLAD, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<номер авто к клиенту>',
                          NVL(V_CAR_NUMBER_C, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<фамилия к клиенту>',
                          NVL(V_DRIVER_NAME_C, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата убытия к клиенту>',
                          NVL(TO_CHAR(V_DATE_C, 'DD.MM.YYYY'), 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<телефон к клиенту>',
                          NVL(V_DRIVER_PHONE_C, 'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<дата подачи>',
                          NVL(TO_CHAR(V_DATE_POD, 'DD.MM.YYYY HH24:MI'),
                              'N/A'));
    V_MSG_INFO := REPLACE(V_MSG_INFO,
                          '<груз по заявке>',
                          NVL(V_gruz_po_zayavke, 'N/A'));
  ...
  BEGIN
    ...
    if P_orst_tmms = 0 then
      select count(ms.clnt_clnt_id || ms.tm_tm_id)
       ...
       where ms.clnt_clnt_id in
             (select cr.clnt_clnt_id
                from clrq_orders co, client_requests cr
               where co.clrq_clrq_id = cr.clrq_id
                 and co.ord_ord_id = P_ORD_ORD_ID)
         and ms.tm_tm_id in
             (select tm_tm_id from order_statuses where orst_id = P_ORST_ID)
    ...
    if clnt_id_to_be_processed > 0 then
      ...
      Begin
        if P_orst_tmms = 0 then
          select TM.MESSAGES_TEXT, TM.STORE_DAYS, TM.is_order
           ...
           where OS.ORST_ID = P_ORST_ID
             and TM.DEL_DATE is null
             and OS.TM_TM_ID = TM.TM_ID;
        elsif P_orst_tmms = 1 then
          select TM.MESSAGES_TEXT, TM.STORE_DAYS, TM.is_order
            INTO V_MESSAGE_TEXT, V_STORE_DAYS, V_is_order -- Текст сообщения и срок
            from TEMPLATES_MESSAGES TM
           where TM.DEL_DATE is null
             and TM.TM_ID = P_ORST_ID;
        end if;
        -- Проверка на повтор сообщения
        select Count(*)
          into nn
          from messages2customers MC
         where ord_ord_id = P_ORD_ORD_ID
           and event_id = P_ORST_ID;
        -- 04/05/2016
        ...
        if nn = 0 then
          if (P_ORST_ID = 1 and V_is_order = 1) then
            ...
            select count(MC.clrq_clrq_id)
             ...
             where co.clrq_clrq_id = MC.clrq_clrq_id
               and co.ord_ord_id = P_ORD_ORD_ID
               and MC.Event_Id = P_ORST_ID;
      ...
      EXCEPTION
        WHEN OTHERS THEN
          RaiseError('Код ошибки ' || SQLERRM || ' Заказ = ' ||
                     P_ORD_ORD_ID);
      end;
      ...
      if P_orst_tmms = 0 then
        select MC.ORD_ORD_ID, TM.MESSAGES_TEXT, tm.def
          ...
          from ORDER_STATUSES     OS,
               TEMPLATES_MESSAGES TM,
               MESSAGES2CUSTOMERS MC
         where MC.MSCM_ID = P_MSCM_ID
           AND OS.ORST_ID = MC.EVENT_ID
           and TM.DEL_DATE is null
           and OS.TM_TM_ID = TM.TM_ID;
      elsif P_orst_tmms = 1 then
        select MC.ORD_ORD_ID, TM.MESSAGES_TEXT, tm.def
         ...
         where MC.MSCM_ID = P_MSCM_ID
           AND MC.EVENT_ID = TM.TM_ID
           and TM.DEL_DATE is null;
      end if;
    EXCEPTION
      WHEN OTHERS THEN
        ins_sys_logs(ApplId   => ApplId,
                     Message  => 'Не работает выборка из ORDER_STATUSES OS, TEMPLATES_MESSAGES TM, MESSAGES2CUSTOMERS MC, P_MSCM_ID=' ||
                                 P_MSCM_ID || ' P_CLNT_CLNT_ID=' ||
                                 P_CLNT_CLNT_ID,
                     IsCommit => True);
    end;
    /*
    ...
    */
    begin
      Select c.HOLD_HOLD_ID, c.clnt_id
       ...
       WHERE O.ORD_ID = V_ORD_ID
         AND O.CLNT_CLNT_ID = C.CLNT_ID
         AND C.DEL_DATE is null;
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RaiseError('Возможно у заказа клиент помечен на удаление!! V_ORD_ID=' ||
                   V_ORD_ID);
    end;
    ...
    P_tm_ms_def    := rtrim(Substr(SET_VALUES_MESSAGES(V_ORD_ID,
                                                       P_tm_ms_def),
                                   1,
                                   199));
  ...
  BEGIN
    ...
    LOOP
      ...
      if INSTR(str2, '</b>') + 4 > LENGTH(str2) Then
        EXIT;
      end if;
      -- Проверка на null
      if NVL(LENGTH(str2), 0) = 0 Then
        EXIT;
      end if;
      str2 := SUBSTR(str2, INSTR(str2, '</b>') + 4, LENGTH(str2));
      -- Если блоков больше нет
      if (INSTR(str2, '<b>') + INSTR(str2, '</b>')) < 2 Then
        Exit;
      end if;
    end LOOP;
  ...
  begin
    ...
    For DAN in (select *
                  from (select MC.MSCM_ID,
                               ...
                               MC.CLNT_CLNT_ID,
                               MC.orst_tmms,
                               SBC_MESSAGE.emails2line(o.ORD_ID) cl_Email,
                               row_number() over(order by MC.message_date) nn
                          from messages2customers MC,
                               orders             o,
                               CLIENT_CONTACTS    CL
                         where mc.ORD_ORD_ID = o.ORD_ID
                           AND CL.CLCN_ID(+) = O.CLCN_CLCN_ID
                           and MC.send_date is null
                           and -- Письмо не отправленно
                               MC.send_to is not null
                           and -- Нет почтового ящика
                               (MC.message_date + MC.STORE_DAYS) > sysdate
                           and -- Количество дней отправки сообщения
                               UPPER(MC.message_text) <> 'NOT'
                           and EVENT_ID not in (12009, 12008)
                           and (((select COMPLETE_DATE
                                    From orders
                                   where ORD_ID = MC.ord_ord_id) is null) OR
                               ((select COMPLETE_DATE
                                    From orders
                                   where ORD_ID = MC.ord_ord_id) > sysdate))
                           and -- Заказ не закрыт (дата фактического завершения заказа)
                               NOT ((MC.SEND_TO = 'message_text') and
                                (Trim(CL.Email) is null))
                         order by MC.message_date)
                 where nn < 100) loop
      V_send_to_last := '';
      For emails_id in (select substr(e_m_fl.s, 1, instr(e_m_fl.s, '^') - 1) e_m,
                               substr(e_m_fl.s, instr(e_m_fl.s, '^') + 1) fl
                          from (select regexp_substr(DAN.cl_Email,
                                                     '[^,]+',
                                                     1,
                                                     level) s
                                  from dual
                                connect by regexp_substr(DAN.cl_Email,
                                                         '[^,]+',
                                                         1,
                                                         level) is not null) e_m_fl) loop
        ...
        if (V_message_text = 'message_text') or
           (V_send_to = 'message_text') then
          Begin
          ...
          EXCEPTION
            WHEN OTHERS THEN
              ins_sys_logs_autonomous(ApplId  => 21,
                                      Message => SQLERRM ||
                                                 ' Ошибка работы EXE_Message процедуры по MSCM_ID=' ||
                                                 Dan.MSCM_ID,
                                      LogDate => sysdate);
          end;
        end if;
        ...
        if (trim(V_CLNT_CLNT_ID) IS NULL) THEN
          V_CLNT_CLNT_ID := CL_ID;
        end if;
        if (trim(V_send_to) IS NULL) OR (trim(V_send_to) = 'message_text') THEN
          V_send_to := emails_id.e_m;
        end if;
        ...
        if V_message_text = 'NOT' then
          Begin
            UPDATE messages2customers
               SET send_date     = sysdate,
                   ...
                   message_thema = cast(substr(V_tm_ms_def, 1, 199) as
                                        varchar(200))
          ...
          EXCEPTION
            WHEN OTHERS THEN
              ins_sys_logs_autonomous(ApplId  => 21,
                                      Message => SQLERRM ||
                                                 ' не может сделать UPDATE messages2customers по MSCM_ID=' ||
                                                 Dan.MSCM_ID ||
                                                 ' но НЕ ВСЕ параметры определены send_to=' ||
                                                 V_send_to_last ||
                                                 ' CLNT_CLNT_ID=' ||
                                                 V_CLNT_CLNT_ID ||
                                                 ' message_text=' ||
        ...
        if (V_message_text <> 'message_text') AND
           (V_send_to <> 'message_text') AND (V_message_text <> 'NOT') AND
           (V_message_text <> '0') Then
          if SBC_SendMail(p_rcvr_name  => V_fl_name, -- 'Клиент',
            ...
            if V_povtor_email = 0 or nvl(V_povtor_email, 0) = 0 then
              V_send_to_last := ltrim(V_send_to_last || ', ' ||
                                      emails_id.e_m,
                                      ', ');
            end if;
            Begin
              ...
              WHEN OTHERS THEN
                ins_sys_logs_autonomous(ApplId  => 21,
                                        Message => SQLERRM ||
                                                   ' не может сделать UPDATE messages2customers по MSCM_ID=' ||
                                                   Dan.MSCM_ID ||
                                                   ' но все параметры определены send_to=' ||
                                                   V_send_to_last ||
                                                   ' CLNT_CLNT_ID=' ||
                                                   V_CLNT_CLNT_ID ||
                                                   ' message_text=' ||
  ...
  EXCEPTION
    WHEN OTHERS THEN
      HandleError(-20001,
                  ' процедура которая должна была удалить старые сообщения клиентам из таблицы messages2customers НЕ сработала ' ||
                  rtrim(to_char(sysdate, 'dd.mm.yyyy hh24:mi:ss')),
                  'del_old_mails_f_mess2cust');
  end Del_old_mails_f_mess2cust;

  --*************************************************************************************************
  -- Перевод значений представленных в виде столбца в линейные
  --*************************************************************************************************
  FUNCTION emails2line(P_ORD_ID IN T_ORDERS.ORD_ID%TYPE) RETURN VARCHAR2 IS
    RES VARCHAR2(2000);
  BEGIN
    Res := '';
    FOR C IN (select email || '^' || first_name || ' ' || last_name email
               ...
               where clcn_id in
                     (select clco.clcn_clcn_id
                        from orders od, clrq_orders clor, clrq_contacts clco
                       where od.ord_id = clor.ord_ord_id
                         and clor.clrq_clrq_id = clco.clrq_clrq_id
                         and send_message = 1
                         and od.ord_id = P_ORD_ID)
               ORDER BY ROWID) LOOP
      RES := RES || C.email || ',';
    END LOOP;
    RETURN(RTRIM(RES, ','));
