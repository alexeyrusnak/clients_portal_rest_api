create or replace package SBC_MAIL is
  ...
  -- Перекодировка
  --*************************************************************************************************
  function Encode_str(str in varchar2, tp in varchar2 default 'Q')
    return varchar2;

  --*************************************************************************************************
  -- Генерация списка получателей
  --*************************************************************************************************
  function Create_rcvr_list(mailto in varchar2) return tbl_Receivers;
  --******************************************************************************************************************************
  -- Отправка почты
  --******************************************************************************************************************************
...
create or replace package body SBC_MAIL is
  ...
  begin
    if tp = 'B' then
      RETURN '=?utf-8?b?' || UTL_RAW.cast_to_varchar2(UTL_ENCODE.base64_encode(UTL_RAW.cast_to_raw(CONVERT(SUBSTR(str,
                                                                                                                  1,
                                                                                                                  24),
                                                                                                           'UTF8')))) || '?=' || CASE WHEN SUBSTR(str,
                                                                                                                                                  25) IS NOT NULL THEN utl_tcp.CRLF || ' ' || ENCODE_STR(SUBSTR(str,
                                                                                                                                                                                                                25),
                                                                                                                                                                                                         tp) END;
    ELSIF tp = 'Q' THEN
      RETURN '=?utf-8?q?' || UTL_RAW.cast_to_varchar2(utl_encode.QUOTED_PRINTABLE_ENCODE(utl_raw.cast_to_raw(CONVERT(SUBSTR(str,
                                                                                                                            1,
                                                                                                                            8),
                                                                                                                     'UTF8')))) || '?=' || CASE WHEN SUBSTR(str,
                                                                                                                                                            9) IS NOT NULL THEN utl_tcp.CRLF || ' ' || ENCODE_STR(SUBSTR(str,
                                                                                                                                                                                                                         9),
                                                                                                                                                                                                                  tp) END;
    ELSE
      RETURN str;
  ...
  procedure Add_Attachment(blobloc     IN blob,
                           ...
                           contentid   IN varchar2 default '',
                           mimetype    IN varchar2 DEFAULT 'text/html',
                           disposition IN varchar2 DEFAULT DISPOSITION_ATTACHMENT) is
  begin
    attachments.extend;
  ...
  -- Генерация списка получателей
  --*************************************************************************************************
  function Create_rcvr_list(mailto in varchar2) return tbl_Receivers is
  ...
  begin
    FOR maxrcptnts IN 1 .. 50 LOOP
      pntr := INSTR(v_mailto, ',');
      buf  := substr(v_mailto, 1, pntr - 1);
      IF pntr > 0 THEN
        IF INSTR(buf, '<') > 0 AND INSTR(buf, '>') > 0 THEN
          rcvr_mail := SUBSTR(buf,
                              INSTR(buf, '<') + 1,
                              INSTR(SUBSTR(buf, INSTR(buf, '<') + 1), '>') - 1);
          IF rcvr_mail IS NOT NULL THEN
            rcvr_list.extend;
            rcvr_list(rcvr_list.count).rcvr_mail := trim(rcvr_mail);
            rcvr_list(rcvr_list.count).rcvr_name := trim(SUBSTR(buf,
                                                                1,
                                                                INSTR(buf,
                                                                      '<') - 1));
          END IF;
  ...
  function Send_Mail(p_rcvr_email       in varchar2, -- Список почтовых ящиков адресатов
    ...
    multipart_mimetype varchar2(256) := 'multipart/mixed; boundary="' ||
                                        boundary || '"';
    ...
    nn            number;
    amt CONSTANT BINARY_INTEGER := 10368; -- 48bytes binary convert to 128bytes of base64. (32767/2 mafor raw convert)
  ...
  begin
    BEGIN
      if p_sys_log_marker is null then
        vSys_log_marker := '';
      else
        vSys_log_marker := '(' || p_sys_log_marker || ') ';
      end if;
      --****************************************************************************************************************
      V_RES := 1;
      ...
      if (trim(P_MES_WALLET_PATH) IS NOT NULL) then
        MES_WALLET_PATH := P_MES_WALLET_PATH;
      end if;
      if (trim(P_MES_WALLET_PASS) IS NOT NULL) then
        MES_WALLET_PASS := P_MES_WALLET_PASS;
      end if;
      --*******************************************************************************************************************
      IF MES_WALLET_PATH is null THEN
        -- Порт 25
        ...
        */
        ...
        if MES_EMAIL_AUTH then
          ...
          utl_smtp.command(conn,
                           utl_raw.cast_to_varchar2(utl_encode.base64_encode(utl_raw.cast_to_raw(MES_EMAIL_FROM))));
          utl_smtp.command(conn,
                           utl_raw.cast_to_varchar2(utl_encode.base64_encode(utl_raw.cast_to_raw(MES_EMAIL_PASSW))));
        end if;
        -- ELHO
      ...
      ELSE
        ...
        if vReply.code != 220 then
          ins_sys_logs(applid   => p_appl_id,
                       message  => vSys_log_marker ||
                                   'utl_smtp.open_connection: ' ||
                                   vReply.code || ' - ' || vReply.text,
                       iscommit => True);
          Return 1;
        end if;
        -- AUTO LOGIN
        ...
        */
        ...
        if vReply.code != 220 then
          ins_sys_logs(applid   => p_appl_id,
                       message  => vSys_log_marker || 'utl_smtp.starttls: ' ||
                                   vReply.code || ' - ' || vReply.text,
                       iscommit => True);
          Return 1;
        end if;
      ...
      for nn in 1 .. vRcpt_list_to.count loop
        ...
        if vRcpt_list_to(nn).rcvr_name is not null then
          utl_smtp.write_data(conn,
                              Encode_str(vRcpt_list_to(nn).rcvr_name) || ' <' || vRcpt_list_to(nn)
                              .rcvr_mail || '>');
        else
      ...
      IF vRcpt_list_copy.count > 0 then
        ...
        FOR nn IN 1 .. vRcpt_list_copy.count LOOP
          ...
          if vRcpt_list_copy(nn).rcvr_name is not null then
            utl_smtp.write_data(conn,
                                Encode_str(vRcpt_list_copy(nn).rcvr_name) || ' <' || vRcpt_list_copy(nn)
                                .rcvr_mail || '>');
          else
      ...
      utl_smtp.write_raw_data(conn,
                              utl_raw.cast_to_raw('Subject:' ||
                                                  ENCODE_STR(p_subject)));
      utl_smtp.write_data(conn, utl_tcp.crlf);
      utl_smtp.write_data(conn, 'MIME-version: 1.0' || utl_tcp.CRLF);
      utl_smtp.write_data(conn,
                          'Content-Type: ' || multipart_mimetype ||
                          utl_tcp.CRLF);
      utl_smtp.write_data(conn, utl_tcp.crlf);
      utl_smtp.write_data(conn,
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ins_sys_logs(ApplId   => p_appl_id,
                       Message  => vSys_log_marker ||
                                   'Не работает Select VALUE_STRING into local_RU_cp from app_parameters_dic where prmt_id = 2115',
                       IsCommit => True);
      end;
      ...
      select replace(text_mimetype, '~', default_RU_cp)
        into text_mimetype
        from dual;
    
      utl_smtp.write_data(conn, first_boundary || utl_tcp.CRLF);
      utl_smtp.write_data(conn,
                          'Content-Type: ' || text_mimetype || utl_tcp.CRLF);
      utl_smtp.write_data(conn,
      ...
      IF attachments.count > 0 THEN
        FOR x IN 1 .. attachments.count LOOP
          ...
          utl_smtp.write_data(conn,
                              'Content-Type: ' || attachments(x).mimetype || ';' ||
                              utl_tcp.crlf);
          utl_smtp.write_data(conn, ' name="');
          utl_smtp.write_raw_data(conn,
                                  utl_raw.cast_to_raw(attachments(x).name));
          utl_smtp.write_data(conn, '"' || utl_tcp.crlf);
          ...
          utl_smtp.write_data(conn,
                              'Content-ID: <' || attachments(x).contentid || '>' ||
                              utl_tcp.crlf);
        
          IF attachments(x)
           .disposition in (DISPOSITION_ATTACHMENT, DISPOSITION_INLINE) THEN
            utl_smtp.write_data(conn,
                                'Content-Disposition: ' || attachments(x)
                                .disposition || ';' || utl_tcp.crlf);
          ...
          IF attachments(x).attachtype = 'FILE' THEN
            vFile := BFILENAME(attachments(x).dirname,
                               attachments(x).filename);
          ...
          ELSIF attachments(x).attachtype = 'CLOB' THEN
            ...
            LOOP
              BEGIN
                ...
                ps := ps + v_amt;
                UTL_SMTP.write_raw_data(conn,
                                        UTL_ENCODE.base64_encode(UTL_RAW.CAST_TO_RAW(vBuf)));
              EXCEPTION
                WHEN NO_DATA_FOUND THEN
                  EXIT;
              END;
    ...
    exception
      when others then
        begin
          Attachments := tbl_Attachments();
          ins_sys_logs(applid   => p_appl_id,
                       message  => vSys_log_marker || 'SBC_Mail.Send_Mail: ' ||
                                   sqlcode || ' - ' || sqlerrm,
                       iscommit => True);
          V_res := 1;
          Return v_Res;
